...
function addExpressionNodes(ast, options) {
  const expressionMap = options.expressionMap;
  ...
  ast.walk((node) => {
    ...
    switch (node.type) {
      case 'element': {
        ...
        console.log(node.name, tok)
        // console.log('tokes', tok)
        if (tok) {
          // console.log(tok)

          // TODO: do I need this code, do I need else/end in expressionMap?
          const closing = expressionMap.get(`</${node.name}>`, {});
          const printClosing = closing ? closing.print : undefined;
          Object.assign(node, {
            type: 'expression',
            printExpression: tok.print,
            expressionType: tok.type,
            // printClosing,
            // openingTag: tok.openingTag,
            // closingTag: tok.closingTag
          });
        }
